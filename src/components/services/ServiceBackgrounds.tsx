...
export function BgDashboard({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <rect x="20" y="20" width="360" height="220" rx="14" fill="none" stroke={STROKE} strokeWidth="1.5"/>
      <line x1="20" y1="60" x2="380" y2="60" stroke={STROKE} strokeWidth="1"/>
      <circle cx="38" cy="40" r="4" fill={STROKE}/><circle cx="52" cy="40" r="4" fill={STROKE}/><circle cx="66" cy="40" r="4" fill={STROKE}/>
      {[0,1,2,3,4,5,6].map(i => (
        <rect key={i} x={50 + i*42} y={200 - (20 + (i%3)*30 + (i*7)%40)} width="22" height={20 + (i%3)*30 + (i*7)%40} rx="3" fill={STROKE} opacity="0.55"/>
      ))}
      <polyline points="50,150 92,120 134,135 176,90 218,105 260,70 302,80 344,50" fill="none" stroke={STROKE} strokeWidth="2"/>
      {[ [50,150],[92,120],[134,135],[176,90],[218,105],[260,70],[302,80],[344,50] ].map(([x,y],i)=>(
...
export function BgSeo({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <polyline points="30,210 90,180 150,190 210,140 270,120 330,70 380,55" fill="none" stroke={STROKE} strokeWidth="2"/>
      <polyline points="30,210 380,210" stroke={STROKE} strokeWidth="1"/>
      {[1,2,3,4,5,6,7].map(i => <line key={i} x1={30+i*50} y1="210" x2={30+i*50} y2="215" stroke={STROKE} strokeWidth="1"/>)}
      <circle cx="300" cy="110" r="44" fill="none" stroke={STROKE} strokeWidth="2.5"/>
      <line x1="334" y1="142" x2="370" y2="180" stroke={STROKE} strokeWidth="3" strokeLinecap="round"/>
      <text x="285" y="118" fontSize="28" fontWeight="700" fill={STROKE}>#1</text>
    </svg>
  );
}
...
export function BgFacebook({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <rect x="40" y="40" width="200" height="80" rx="8" fill="none" stroke={STROKE} strokeWidth="1.5"/>
      ...
      <rect x="40" y="140" width="200" height="80" rx="8" fill="none" stroke={STROKE} strokeWidth="1.5"/>
      ...
      <g transform="translate(290,90)">
        <path d="M0 30 V60 a4 4 0 0 0 4 4 H30 a6 6 0 0 0 6-5 l4-20 a4 4 0 0 0 -4-5 H22 l3-12 a6 6 0 0 0 -10-5 L0 30 Z" fill={STROKE} opacity="0.55"/>
      </g>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      {[0,1,2].map(r=>[0,1,2,3].map(c=>(
        <rect key={`${r}-${c}`} x={40+c*60} y={30+r*60} width="50" height="50" rx="6" fill="none" stroke={STROKE} strokeWidth="1.5"/>
      )))}
      ...
      <path d="M325 195 c-18-14 -30-26 -30-42 a16 16 0 0 1 30-8 a16 16 0 0 1 30 8 c0 16 -12 28 -30 42 Z" fill={STROKE} opacity="0.55"/>
    </svg>
  );
}
...
export function BgYoutube({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <rect x="60" y="50" width="180" height="120" rx="14" fill="none" stroke={STROKE} strokeWidth="2"/>
      ...
      {Array.from({length:14}).map((_,i)=>(
        <rect key={i} x={270+i*8} y={130-((i*13)%40)} width="4" height={20+((i*17)%60)} rx="1" fill={STROKE} opacity="0.55"/>
      ))}
    </svg>
  );
}
...
export function BgWebsite({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <rect x="30" y="30" width="340" height="200" rx="12" fill="none" stroke={STROKE} strokeWidth="2"/>
      <line x1="30" y1="60" x2="370" y2="60" stroke={STROKE} strokeWidth="1.5"/>
      <circle cx="48" cy="45" r="4" fill={STROKE}/><circle cx="62" cy="45" r="4" fill={STROKE}/><circle cx="76" cy="45" r="4" fill={STROKE}/>
      ...
      <rect x="220" y="80" width="130" height="120" rx="8" fill="none" stroke={STROKE} strokeWidth="1.5"/>
...
export function BgMap({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <rect x="30" y="30" width="340" height="200" rx="12" fill="none" stroke={STROKE} strokeWidth="1.5"/>
      <path d="M30 100 Q 150 80 250 130 T 370 150" fill="none" stroke={STROKE} strokeWidth="1.2" opacity="0.5"/>
      <path d="M30 170 Q 130 200 220 180 T 370 200" fill="none" stroke={STROKE} strokeWidth="1.2" opacity="0.5"/>
      <path d="M120 30 Q 140 130 100 230" fill="none" stroke={STROKE} strokeWidth="1.2" opacity="0.4"/>
      <path d="M280 30 Q 260 120 300 230" fill="none" stroke={STROKE} strokeWidth="1.2" opacity="0.4"/>
      {[ [120,90],[230,140],[300,80],[160,180] ].map(([x,y],i)=>(
        <g key={i} transform={`translate(${x},${y})`}>
          <path d="M0 0 c-10 0 -18 8 -18 18 c0 13 18 30 18 30 s18-17 18-30 c0-10 -8-18 -18-18 Z" fill={STROKE} opacity="0.55"/>
...
export function BgShopping({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <path d="M120 90 h120 l-12 130 a10 10 0 0 1 -10 9 H142 a10 10 0 0 1 -10-9 Z" fill="none" stroke={STROKE} strokeWidth="2"/>
...
export function BgFunnel({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <path d="M80 50 H320 L230 130 V210 L170 180 V130 Z" fill="none" stroke={STROKE} strokeWidth="2"/>
      ...
      {[ [60,70],[340,60],[50,130],[350,140],[70,200] ].map(([x,y],i)=>(
        <circle key={i} cx={x} cy={y} r="6" fill={STROKE} opacity="0.5"/>
      ))}
      <path d="M200 220 l0 20 M190 232 l10 8 10-8" fill="none" stroke={STROKE} strokeWidth="2" opacity="0.6"/>
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <g transform="translate(160,80)">
        <path d="M10 0 h40 a10 10 0 0 1 10 10 l8 30 a8 8 0 0 1 -4 9 l-14 8 a60 60 0 0 0 30 30 l8-14 a8 8 0 0 1 9-4 l30 8 a10 10 0 0 1 10 10 v40 a10 10 0 0 1 -10 10 C 50 137 -7 80 0 10 A 10 10 0 0 1 10 0 Z" fill={STROKE} opacity="0.55"/>
      </g>
      {[40,60,80].map((r,i)=>(
        <path key={i} d={`M60 130 a${r} ${r} 0 0 1 ${r*2} 0`} fill="none" stroke={STROKE} strokeWidth="2" opacity={0.6-i*0.15}/>
      ))}
      {[40,60,80].map((r,i)=>(
        <path key={i} d={`M340 130 a${r} ${r} 0 0 0 -${r*2} 0`} fill="none" stroke={STROKE} strokeWidth="2" opacity={0.6-i*0.15}/>
      ))}
    </svg>
  );
}
  ...
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <g transform="translate(200,130)">
        {[100,75,50,25].map(r=> <circle key={r} cx="0" cy="0" r={r} fill="none" stroke={STROKE} strokeWidth="1.2"/>)}
      ...
      {[ [120,80],[260,180],[300,90],[170,200],[230,70] ].map(([x,y],i)=>(
...
export function BgShield({ className }: Props) {
  return (
    <svg viewBox="0 0 400 260" className={className} aria-hidden>
      <path d="M200 30 L300 70 V140 C 300 190 250 220 200 235 C 150 220 100 190 100 140 V70 Z" fill="none" stroke={STROKE} strokeWidth="2"/>
      <path d="M160 135 l30 30 l60-60" fill="none" stroke={STROKE} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
