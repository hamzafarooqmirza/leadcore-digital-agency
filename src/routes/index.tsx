...
import {
  ArrowRight, Phone, MessageCircle, BadgeCheck, BarChart3, Target, Rocket,
  Globe, Search, MousePointerClick, Megaphone, ShoppingBag, PhoneCall,
  LineChart, ShieldCheck, Sparkles, Star, CheckCircle2, Zap, Users, TrendingUp,
} from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Generate more leads, calls and sales with Google Ads, SEO, web design, conversion tracking and social media marketing by LeadCore Digital." },
      { property: "og:title", content: "LeadCore Digital — Your Growth Partner In Digital Marketing" },
      { property: "og:description", content: "Generate more leads, calls and sales with Google Ads, SEO, web design and social media marketing." },
  ...
  return (
    <SiteShell>
        ...
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 pt-20 pb-24 lg:pt-28 lg:pb-32 relative">
          <div className="max-w-3xl">
            ...
            <h1 className="mt-6 font-display text-4xl sm:text-5xl lg:text-6xl font-bold leading-[1.05]">
              Grow Your Business With <span className="text-gradient-gold">Data-Driven</span> Digital Marketing
            </h1>
            <p className="mt-6 text-lg text-muted-foreground max-w-2xl">
              LeadCore Digital helps businesses generate more leads, calls and sales through
              Google Ads, SEO, Website Design, Conversion Tracking and Social Media Marketing.
            </p>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <div key={s.l} className="card-elevated rounded-2xl p-6 text-center">
                <div className="text-3xl lg:text-4xl font-display font-bold text-gradient-gold">{s.v}</div>
      ...
      <section id="services" className="container mx-auto max-w-7xl px-4 sm:px-6 py-20">
        <SectionHead eyebrow="Our Services" title="Premium Marketing Built To Convert" desc="End-to-end digital marketing services engineered to drive measurable growth for your business." />
      ...
      <section className="bg-surface border-y border-border">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 py-20">
          <SectionHead eyebrow="Why Choose Us" title="A Growth Partner You Can Trust" desc="We don't sell impressions — we deliver leads, calls and revenue." />
      ...
      <section id="results" className="container mx-auto max-w-7xl px-4 sm:px-6 py-20">
        ...
        <div className="mt-10 text-center">
          <Link to="/projects" className="inline-flex items-center gap-2 rounded-full border border-gold/40 bg-gold/10 px-6 py-3 text-sm font-semibold text-gold hover:bg-gold hover:text-gold-foreground transition">
      ...
      <section className="bg-surface border-y border-border">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 py-20">
          <SectionHead eyebrow="Our Process" title="From Strategy To Scale" desc="A proven five-step framework we apply to every client engagement." />
          <ol className="mt-12 grid md:grid-cols-5 gap-4">
            {[
              "Business Analysis", "Strategy Planning", "Campaign Setup", "Optimization", "Reporting & Growth",
            ].map((step, i) => (
              <li key={step} className="relative rounded-2xl p-6 bg-background/40 border border-border">
      ...
      <section className="container mx-auto max-w-7xl px-4 sm:px-6 py-20">
        <SectionHead eyebrow="Industries We Serve" title="Specialists Across High-Intent Verticals" desc="We have a track record of generating leads across these industries." />
        <div className="mt-10 flex flex-wrap gap-3">
          {industries.map((ind) => (
            <span key={ind} className="rounded-full border border-border bg-surface px-4 py-2 text-sm text-muted-foreground hover:border-gold hover:text-gold transition">
              {ind}
            </span>
          ))}
        </div>
      ...
      <section className="bg-surface border-y border-border">
        <div className="container mx-auto max-w-7xl px-4 sm:px-6 py-20">
          <SectionHead eyebrow="Testimonials" title="What Our Clients Say" desc="Real stories from business owners who trusted LeadCore Digital." />
          <div className="mt-12 grid md:grid-cols-3 gap-5">
            {[
              { n: "James M.", b: "ResQ Tyres, UK", q: "LeadCore Digital transformed our Google Ads. We went from a handful of calls to 160+ in a single day." },
              { n: "Sara A.", b: "Bright Clean Co.", q: "Transparent reporting, fast support and real results. Our cost per lead dropped 60% in 2 months." },
              { n: "Hassan K.", b: "Premier Recovery", q: "Best agency we've worked with. Their PPC and call-tracking setup is next level." },
            ].map((t) => (
        ...
        <div className="relative overflow-hidden rounded-3xl card-elevated p-10 lg:p-14 text-center">
          ...
          <div className="relative">
            ...
            <p className="mt-3 text-muted-foreground max-w-xl mx-auto">
              Get a free, no-obligation audit of your Google Ads, SEO and website — and discover how many more leads you could be generating.
            </p>
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <Link to="/audit" className="inline-flex items-center gap-2 rounded-full bg-gold px-6 py-3 text-sm font-semibold text-gold-foreground hover:brightness-110 glow-gold">
                Get Your Free Audit <ArrowRight className="h-4 w-4" />
              </Link>
              <a href={`tel:${site.phoneRaw}`} className="inline-flex items-center gap-2 rounded-full border border-border bg-background px-6 py-3 text-sm font-semibold hover:border-gold">
