...
export const Route = createFileRoute("/services/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      ...
      {
        name: "description",
        content: `${loaderData?.title} — ${loaderData?.short} Get a free strategy call with LeadCore Digital.`,
      },
  ...
  notFoundComponent: () => (
    <SiteShell>
      <div className="container max-w-3xl mx-auto px-6 py-32 text-center">
        <h1 className="text-3xl font-display font-bold">Service not found</h1>
        <Link to="/services" className="mt-4 inline-block text-gold">
          Browse services →
        </Link>
      </div>
    </SiteShell>
...
function ServiceDetail() {
  ...
  const faqs = [
    {
      q: `How quickly will I see results from ${svc.title}?`,
      a: "Most clients see measurable lift within the first 2–4 weeks. SEO compounds over 3–6 months.",
    },
    {
      q: "What budget do I need to get started?",
      a: "We work with budgets of every size. We'll recommend an honest starting point during your free consultation.",
    },
    {
      q: "Do you provide reporting?",
      a: "Yes — every client gets a transparent weekly report with leads, calls, spend and ROI.",
    },
    {
      q: "How do I get started?",
      a: "Book a free audit or chat with us on WhatsApp. We'll review your situation and propose a plan within 24 hours.",
    },
  ];
  ...
  return (
    <SiteShell>
      <section className="container mx-auto max-w-5xl px-4 sm:px-6 pt-20 pb-10">
        <Link
          to="/services"
          className="text-xs uppercase tracking-[0.18em] text-gold font-semibold"
        >
          ← All Services
        </Link>
        ...
        <div className="mt-7 flex flex-wrap gap-3">
          <Link
            to="/audit"
            className="inline-flex items-center gap-2 rounded-full bg-gold px-6 py-3 text-sm font-semibold text-gold-foreground glow-gold"
          >
            Get Free Audit <ArrowRight className="h-4 w-4" />
          </Link>
          <a
            href={whatsappUrl(`Hi, I'm interested in ${svc.title}.`)}
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 rounded-full border border-border bg-surface px-6 py-3 text-sm font-semibold hover:border-gold"
          >
      ...
      <section className="container mx-auto max-w-5xl px-4 sm:px-6 py-12 grid lg:grid-cols-2 gap-8">
        <div className="card-elevated rounded-2xl p-7">
          ...
          <p className="mt-3 text-muted-foreground">
            Our {svc.title} service is engineered for one thing: results. We combine certified
            expertise, data-driven optimization and creative excellence to drive measurable growth
            for your business.
          </p>
        </div>
        <div className="card-elevated rounded-2xl p-7">
        ...
        <section className="container mx-auto max-w-6xl px-4 sm:px-6 py-12">
          <div className="mb-8">
            ...
            <p className="mt-3 text-muted-foreground max-w-2xl">
              Actual campaign screenshots — calls generated, cost per lead and conversion efficiency
              from live client accounts.
            </p>
          </div>
      ...
      <section className="container mx-auto max-w-5xl px-4 sm:px-6 py-16">
        <div className="rounded-3xl card-elevated p-10 text-center relative overflow-hidden">
          ...
          <h3 className="relative font-display text-3xl font-bold">
            Ready to grow with {svc.title}?
          </h3>
          <p className="relative mt-3 text-muted-foreground">
            Book a free consultation — we'll map out your growth plan in 30 minutes.
          </p>
